...
class XmlController extends Controller
{
    ...
    public function parser(Request $request)
    {
        ...
        foreach ($result as $items) {
            ...
            $book1->addChild("product_id", $items->xpath('ProductId')[0]);
            $book1->addChild("active", $items->xpath('Status')[0]);
            $book1->addChild("name", $items->xpath('ProductName')[0]);
            $book1->addChild("description", '<![CDATA[' . html_entity_decode($items->xpath('FullDescription')[0]) . ' ]]');
            $book1->addChild("barcode", $items->xpath('ProductGtin')[0]);
            $book1->addChild("stock", $items->xpath('ProductStockQuantity')[0]);
            $book1->addChild("price", $items->xpath('ProductPrice')[0] + ($items->xpath('ProductPrice')[0] * 100) / 100);
           ...
           // $pictures = $book1->addChild("Pictures");
            ...
            if ($x) {
                ...
                $picturesI = implode(",", $yz);
                $book1->addChild("images", $picturesI);
            }



            $varyantarray = $items->xpath('ProductCombinations/ProductCombination');
            $varyants = $book1->addChild("varyants");
            ...
            foreach ($varyantarray as $varyant) {
                ...
                $varyantItem->addChild('price', $items->xpath('ProductPrice')[0] + ($items->xpath('ProductPrice')[0] * 100) / 100);
            ...
            $categorytree->addChild('category_1',$a->xpath('Category')[0]->CategorName);
            $category_data->addChild('category_id');
            $book1->addChild("brand", $b->xpath('Manufacturer')[0]->ManufacturerName ?? 'Akuzun');
    ...
    public function databaseXml()
    {
        ...
        foreach ($resultO as $itemso) {
            ...
            $book2->addChild("name", ''. html_entity_decode($itemso->ProductName).'');
...
/*
    public function myxmlXml()
    {
        ...
        foreach ($result as $items) {
            ...
            $book1->addChild("name", ''.html_entity_decode(ProductContent::where('product_id', $items->product_id)->where('language_id', 1)->first()->name).'');
            ...
            $i = 1;
            foreach ($categorylists as $categorylist) {
                $categorytree->addChild('category_'.$i.'', trim($categorylist));
                $i++;  if ($i == 3) {
                    break;
                }
            }
        ...
        file_put_contents('myxml.xml', $simplexml->asXML());
        return redirect()->back();
    }
    */
}
